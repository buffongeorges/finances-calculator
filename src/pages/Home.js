...
function Home() {
  ...
  return new Date(formattedDate).toLocaleDateString('en-US', {timeZone: 'UTC'});
  };
  ...
  const parseDateArray = (dateString) => {
    if (/^\d{4}/.test(dateString)) { // Vérifie si la chaîne de caractères commence par une année (4 chiffres)
                ...
                return (
                  <>
                    <tr key={index} style={styles}>
                      <td>{parseDateArray(value.Date)}</td>
